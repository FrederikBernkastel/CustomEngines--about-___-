...
namespace Veldrid.OpenGL
{
    internal static class OpenGLUtil
    {
        ...
        {
            uint error = Internal.OpenGL.GetError();
            if (error != 0)
            {
        ...
        internal static unsafe void SetObjectLabel(ObjectLabelIdentifier identifier, uint target, string name)
        {
            ...
            if (MaxLabelLength == null)
            {
                ...
                Internal.OpenGL.GetIntegerv(GetPName.MaxLabelLength, ref maxLabelLength);
                CheckLastError();
            ...
            fixed (byte* utf8bytePtr = utf8bytes)
            {
                ...
                Internal.OpenGL.ObjectLabel(identifier, target, (uint)byteCount, utf8bytePtr);
                CheckLastError();
            }
        }
