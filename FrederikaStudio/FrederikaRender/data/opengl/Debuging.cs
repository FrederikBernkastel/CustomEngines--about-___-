...
public static class Debuging
{
    ...
    public static void Initialization(bool OUTPUT_SYNCHRONOUS = true, IntPtr useParam = default)
    {
        if (Flag)
            return;
        Library.OpenGL.Enable(_nOpenGL.Enable_cap.DebugOutput);
        if (OUTPUT_SYNCHRONOUS)
            Library.OpenGL.Enable(_nOpenGL.Enable_cap.DebugOutputSynchronous);
        Library.OpenGL.DebugMessageCallback(MessageCallback, useParam);
        Flag = true;
        static void MessageCallback(
            _nOpenGL.DebugSource source, _nOpenGL.DebugType type, uint id,
            _nOpenGL.DebugSeverity severity, uint length, IntPtr message, IntPtr userParam)
        {
            ...
            Console.WriteLine(
                $"Debug message ({id}): {Marshal.PtrToStringAnsi(message, (int)length)}\nSource: " +
